...
public class AnnotationsToHTML {
    ...
    public static String getHTMLText(String text, List<Annotation> annotations, boolean removeOverlaps){
        ...
        for (Annotation annotation : annotations){
            String before = result.substring(0, annotation.getSpan().starts + added);
            String after = result.substring(annotation.getSpan().ends + added, original.length() + added);
            String between = result.substring(annotation.getSpan().starts + added, annotation.getSpan().ends + added);
    ...
    private static List<Annotation> sortAnnotations(List<Annotation> annotations){
        ...
        for (Annotation toAdd : annotations){
            ...
            for (Annotation current : sorted){
                if (toAdd.getSpan().starts<current.getSpan().starts){
                    break;
                }else if (toAdd.getSpan().starts==current.getSpan().starts){
                    if (toAdd.getSpan().ends>=current.getSpan().ends){
                        break;
                    }
                }
                i++;
    ...
    private static List<Annotation> removeOverlaps(List<Annotation> annotations){
        ...
        int lastEnd = -1;
        for (Annotation annotation : annotations){
            if (annotation.getSpan().starts>lastEnd){
                results.add(annotation);
                lastEnd = annotation.getSpan().ends;
            }
        }
        return results;
    }
